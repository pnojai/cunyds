...
$\widehat{baby\_weight}$ = 123.05 - 8.94 $\times$ $smoke$
...
The smoke coefficient of -8.94 says that smoking by a mother reduces a baby's birth weight, on average, by 8.94 ounces.

```{r}
bwt_smoke <- 123.05 - 8.94 * 1
bwt_nonsmoke <- 123.05 - 8.94 * 0
```

$\widehat{baby\_weight_{smoke}}$: `r bwt_smoke` ozs.

$\widehat{baby\_weight_{non\_smoke}}$: `r bwt_nonsmoke` ozs.

(c) Is there a statistically significant relationship between the average birth weight and smoking?

Yes, as indicated by the p value of 0 to four decimal places.
...
The `eth` coefficient of -9.11 says that a non-aborigine misses, on average, 9.11 fewer days than an aborigine.

The `sex` coefficient of 3.10 says that male misses, on average, 3.10 days more than a female.

The `lrn` coefficient of 2.15 says that a slow learner misses, on average, 2.15 more days than an average learner.
...
pred <- 18.93 - 9.11 * eth + 3.10 * sex + 2.15 * lrn 
res <- obs - pred
```

Observed days missed: `r obs` days.

Predicted days missed: `r pred` days.
...
We should remove learner status, as its removal yields an adjusted $R^2$ value of 0.0723, which is higher than the adjusted $R^2$ value of 0.0701 for the full model.
...
```{r}
load("orings.rda")
plot(orings$temp, orings$damage)
```

Damage co-occurs with almost all temperatures, with an outlier of high number of damages at the lowest observed temperature.
...
The `Temperature` coefficient of -0.2162 says that a one-degree increase in temperature reduces the probability of o-ring damage by 0.22. The p-value for this predictor is very close to 0, implying its high statistical significance. Restated, the lower the temperature, the higher the probability of o-ring damage
...
Yes, due to the high implied statistical significance of temperature as a predictor.
