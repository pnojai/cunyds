...
**ANSWER**: The trend over the domain of the plot is rising, although there is a pronounced drop during the years 1640-1660.
...
**ANSWER**: The following plot shows that the proportion of boys over time varies, but it is over half of all births in most years. There appears to be one year in which births of boys was less than half.
...
-   How do these counts compare to Arbuthnot's? Are they on a similar scale?
    ...
    **ANSWER**: The dataset, present, reports a much higher scale.
    
    - mean(arbuthnot\$girls + arbuthnot\$boys):  11,441.74
    - mean(present\$girls + present\$boys):  3,679,515
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    **ANSWER**: Yes, Arbuthnot's observation remains true for the period sampled in the U.S, although the disparity between boys and girls appears to run a little smaller in the U.S. data.
    
```{r onyourown_plot, echo = TRUE}
plot(present$year, present$boys / (present$boys + present$girls), type = "l")

mean_ratio_boys_girls_arb = mean(arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls))
mean_ratio_boys_girls_pres = mean(present$boys / (present$boys + present$girls))

print(paste0("Mean ratio of boys to girls, Arbuthnot: ", mean_ratio_boys_girls_arb))
print(paste0("Mean ratio of boys to girls, Present: ", mean_ratio_boys_girls_pres))
```
...
**ANSWER**:    
```{r maxbirth, echo = TRUE}
print(paste0("Year of most total births in the U.S.: ",
             present[which.max(present$boys + present$girls), "year"]))
```

**QUESTION**: I'm a SQL guy, and I confess I always tie myself in knots with these kinds of queries in R. I think my syntax is convoluted. This ought to be easy. How would you do it? *~~Jai*
