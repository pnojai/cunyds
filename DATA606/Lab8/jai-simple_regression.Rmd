...
Scatter plot. I'd like to do better than .61 correlation. Those residuals are going to be high.
...
There is a positive linear relationship between at_bats and runs. The correlation, at .61, is weak. The residuals are distant from the best fit line.
...
My best attempt was my first one, giving 137360.
...
```
\[
  \hat{y} = 415.2389 + 1.83455 * homeruns
\]

The question asks about the slope and the success of a team in relation to its home runs. The slope is 1.8, which says that the score of a team is almost twice the frequency of its home runs.
...
$runs_{pred}$: `r -2789.2429 + 0.6305 * 5578`

I don't understand the definite article in "the residual." There is no team with 5,578 at-bats. The Phillies are close.

```{r}
library(dplyr)
mlb11[between(mlb11$at_bats, 5570, 5580), ]
```

They had 5579 at-bats, very close to 5578. Their runs were 713, so the estimate was over, if you count the residual for the nearest observation, and the amount of that nearby residual was `r 727.6861 - 713`.
...
No pattern. 1. Linear. I think it also supports the conclusion of 3., constant variability.
...
Yes, the scatterplot suggests a linear relationship.
...
plot(m_bat_avg$residuals ~ mlb11$bat_avg)
abline(h = 0, lty = 3)
```
...
R$^2$ for `runs` and `at_bats`: `r summary(m1)$r.squared`.

R$^2$ for `runs` and `bat_avg`: `r summary(m_bat_avg)$r.squared`.

`bat_avg` is a better predictor than `at_bats`.
...
```{r}
for (i in 1:length(names(mlb11))) {
  if (is.numeric(mlb11[, i])) {
    print(names(mlb11)[i])
    mdl <- lm(as.formula(paste0("runs ~ ", names(mlb11[i]))), data = mlb11)
    print(summary(mdl)$r.squared)
  }
}
```

Of the traditional variables, the best predictor for `runs` besides `bat_avg` is `hits`, whose R$^2$ is 0.6419388.

Plotting the model and diagnostics...
...
plot(m_hits$residuals ~ mlb11$hits)
abline(h = 0, lty = 3)

hist(m_hits$residuals)
qqnorm(m_hits$residuals)
qqline(m_hits$residuals)
```
...
summary(m_onbase)$r.squared

m_slug <- lm(runs ~ new_slug, data = mlb11)
summary(m_slug)$r.squared

m_obs <- lm(runs ~ new_obs, data = mlb11)
summary(m_obs)$r.squared
```

The new variables are more effective, as evidenced by higher R$^2$ values. Compare the residuals of two of them. The spread is narrower. Check out the ranges of two residual graphs and the residual standard error, 49.23 for `bat_avg` vs. 32.61 for `new_on_base`.

```{r}
par(mfrow = c(1,2))
# Traditional variable.
plot(m_bat_avg$residuals ~ mlb11$bat_avg, ylim = c(-100, 100))
abline(h = 0, lty = 3)

# New variable
plot(m_onbase$residuals ~ mlb11$new_onbase, ylim = c(-100, 100))
abline(h = 0, lty = 3)
...
```{r}
plot(m_obs$residuals ~ mlb11$new_obs)
abline(h = 0, lty = 3)

hist(m_obs$residuals)
qqnorm(m_obs$residuals)
qqline(m_obs$residuals)
```