...
load_data <- function(df) {
	...
	out$round_df <- read_rounds(df)
	
	return(out)
}
```
## Extract, clean, and transform
```{r}
dat_file_name <- "tournamentinfo.txt"
# Fully qualified path.
dat_file <- paste(assign_data, dat_file_name, sep = "/")

chess_dat_list <- process_file(dat_file)
...
# Count rounds
# Headings are in row 2 of the raw data.
round_count <- str_count(toupper(chess_raw[2, 1]), "ROUND")
...
# Transform
chess_xform <- xform_data(chess_xform)
```

## Load
```{r}
assignment_output <- load_data(chess_dat_list$chess_xform)
...
# Load ROUND data frame.
round_df <- read_rounds(out$chess_xform)
```
